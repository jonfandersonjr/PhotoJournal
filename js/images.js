...
//Call functions for loading images/handlers once page is loaded
$(document).ready(() => {
    loadImages();
    attachMouseEnter();
    attachMouseLeave();
...
//Take each image and load it onto the page with its given title
function loadImages () {
    for (let i = 0; i < images.length; i++) {
        let imgPath = images[i].path;
        let imgTitle = images[i].title;
        $(".gallery").append(`<li><img src="./images/square/${imgPath}"
        alt="${imgTitle}"></li>`);
    }
}

//Loads larger preview when mouse covers an image
function attachMouseEnter () {
    $(".gallery img").mouseenter(function(e) {

        //Gray out the original image
        $(this).addClass("gray");

        //Get the title of this image moused over
        let currTitle = $(this).attr("alt");
        
        //Stoarage for image caption
        let currCountry, currCity, currPath, currTaken;

        //Once we find which image we are on, store its info
        for (let i = 0; i < images.length; i++) {
            if (currTitle === images[i].title) {
                currCountry = images[i].country;
                currCity = images[i].city;
                currPath = images[i].path;
                currTaken = images[i].taken;
                break;
            }
        }
        
        //Create the larger preview image
        $(document.body).append(
            `<div id="preview">
            <img src="./images/medium/${currPath}">
            <p>${currTitle}</p>
            <p><i>${currCity}, ${currCountry} [${currTaken}]</i></p>
            </div>`
        );

        //Adjust where the preview appears on the page
        $("#preview").css({
            left: getX(e.pageX) + "px",
            top: getY(e.pageY) + "px"
        });
        //Load the preview
        $("#preview").fadeIn(1000);
...
//Removes the preview image once the mouse no longer hovers on an image
...
//Update location of preview image as mouse moves
...
//Helper to adjust X location of preview image
function getX (originalX) {
    return originalX + 30;
}

//Helper to adjust Y location of preview image
